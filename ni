	...
	task.spawn(function()
		repeat
			...
			if Debounce == false then
				...
				local Percentage = math.clamp(Position, 0, 0.99)

				local TweenInformation = TweenInfo.new(0.05, Enum.EasingStyle.Linear, Enum.EasingDirection.In, 0, false, 0)
				local PositionTween = TweenService:Create(FillCircle, TweenInformation, {Position = UDim2.new(Percentage - 0.05, 0, BarPosition.Y.Scale, BarPosition.Y.Offset)})
