	...
	if Flying == true then
		...
		task.spawn(function()
			RunLoops:BindToHeartbeat("Fly", function(Delta)
				if IsAlive(LocalPlayer) and Flying and (tick() - FlyGroundTime) < 2.5 then
					local FlyVelocity = LocalPlayer.Character.Humanoid.MoveDirection * 20
					
					LocalPlayer.Character.HumanoidRootPart.Velocity = FlyVelocity + (Vector3.new(0, (FlyUp and 40 or 0) + (FlyDown and 40 or 0), 0))
				end
			end)		
		end)
	end
