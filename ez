...
local FrostHammerHit = AlSploitApi.FrostHammerHit
...
function ShootProjectile(Item, Projectile)
	...
	if NearestPlayer then
		local Args = {
			[1] = ReplicatedStorage.Inventories:FindFirstChild(LocalPlayer.Name):FindFirstChild(Item),
			[2] = Projectile,
			[3] = Projectile,
...
task.spawn(function()
	repeat
		if Settings.AutoKit.Value == true then
			FrostHammerHit:FireServer("FROST_HAMMER_SLAM")
		end
	until not game
end)
		...
		task.spawn(function()
			repeat
				...
				if IsAlive(LocalPlayer) then
					...
					if not NearestNpc or not IsAlive(LocalPlayer) or LocalPlayer.Character:GetAttribute("Health") <= 30 or not IsNetworkOwner(LocalPlayer.Character.HumanoidRootPart) then
