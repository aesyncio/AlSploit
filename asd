	...
	if Flying == true then
		...
		task.spawn(function()
			RunLoops:BindToHeartbeat("Fly", function(Delta)
				if IsAlive(LocalPlayer) and Flying then
					local FlyVelocity = LocalPlayer.Character.Humanoid.MoveDirection * 20
					local Mass = (LocalPlayer.Character.HumanoidRootPart:GetMass() - 1.4) * (Delta * 100)
					

					Mass = Mass + 4 * (tick() % 0.4 < 0.2 and -1 or 1)
					
					LocalPlayer.Character.HumanoidRootPart.Velocity = FlyVelocity + (Vector3.new(0, Mass + (FlyUp and 40 or 0) + (FlyDown and 40 or 0), 0))
				 	LocalPlayer.Character.HumanoidRootPart.CFrame = LocalPlayer.Character.HumanoidRootPart.CFrame + (LocalPlayer.Character.Humanoid.MoveDirection * ((23 + GetSpeed()) - 20)) * Delta
				end
			end)		
		end)
	end
