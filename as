...
function CreateTab(Name)
	...
	Title.Position = UDim2.new(-0.005, 0, 0, 0)
...
local SaveFileName = "AlSploitBedwarsRewrite2"
local SaveFilePath = ""
...
function DeleteUnwantedFiles()
	for i = 1, 1000 do
		if isfile("AlSploitBedwars/" .. "AlSploitBedwarsRewrite" .. i) and not SaveFileName:find(i) then
			deletefile("AlSploitBedwars/" .. "AlSploitBedwarsRewrite" .. i)
		end
	end	
end
...
function CreateSettingsFile()
	...
	SaveFilePath = "AlSploitBedwars/"
		
	writefile(SaveFilePath .. SaveFileName, JsonEncodeSettings)
		
	FirstTime = false
end

function CheckFirstTime()
	if isfile("AlSploitBedwars/" .. SaveFileName) then
		return false
	end
...
local SetObservedChestRemote = AlSploitApi.SetObservedChestRemote
...
local CombatConstant = AlSploitApi.CombatConstant
local PlacementCPS = AlSploitApi.PlacementCPS
local MinerRemote = AlSploitApi.MinerRemote

local KnockBackTable = AlSploitApi.KnockBackTable
local ItemTable = AlSploitApi.ItemTable

local ZephyrController = AlSploitApi.ZephyrController
local SprintController = AlSploitApi.SprintController
	...
	task.spawn(function()
		for i, v in next, Players do
			if IsAlive(v) and v.Team ~= LocalPlayer.Team then
				local Distance = (v.Character.PrimaryPart.Position - LocalPlayer.Character.HumanoidRootPart.Position).Magnitude
...
task.spawn(function()
	local function SwordHit(Entity, Weapon)
		...
		task.spawn(function()
			SelfPosition = LocalPlayerHumanoidRootPart.Position + (20 > 14 and (LocalPlayerHumanoidRootPart.Position - EntityHumanoidRootPart.Position).Magnitude > 14.4 and (CFrame.lookAt(LocalPlayerHumanoidRootPart.Position, EntityHumanoidRootPart.Position).lookVector * ((LocalPlayerHumanoidRootPart.Position - EntityHumanoidRootPart.Position).Magnitude - 14)) or Vector3.zero)
				...
				if Flying == true and IsAlive(LocalPlayer) then
					...
					if (tick() - FlyGroundTime) >= 2 then
						...
						if Raycast and IsAlive(LocalPlayer) then 
							...
							if IsAlive(LocalPlayer) then 
								LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new((LocalPlayer.Character.HumanoidRootPart.Position + Vector3.new(0, FlyTeleportPosition, 0) - Vector3.new(0, LocalPlayer.Character.HumanoidRootPart.Position.Y, 0)))
...
function InfFly()
	...
	if InfFlying == true and IsAlive(LocalPlayer) then
		...
		LocalPlayer.Character.HumanoidRootPart.CFrame += Vector3.new(0, 1000000, 0)
		SpoofedCamera.Position = Vector3.new(SpoofedCamera.Position.X, StartLevel, SpoofedCamera.Position.Z)

		repeat task.wait()
			SpoofedCamera.Position = Vector3.new(LocalPlayer.Character.HumanoidRootPart.Position.X, SpoofedCamera.Position.Y + (InfFlyUp and 1 or 0) + (InfFlyDown and -1 or 0), LocalPlayer.Character.HumanoidRootPart.Position.Z)
			StartLevel = SpoofedCamera.Position.Y	
		until InfFlying == false

		if not InfFlying or not IsAlive(LocalPlayer) then
			LocalPlayer.Character.HumanoidRootPart.CFrame = SpoofedCamera.CFrame + Vector3.new(0, 5, 0)		
		...
		task.spawn(function()
			repeat
				...
				if IsAlive(LocalPlayer) then				
					...
					NearestBed = FindNearestBed(Settings.Nuker.Range)
				end				
			until not game
		end)
...
task.spawn(function()
	local AcDisabler = CreateToggle(BlatantTab, "AcDisabler", Settings.AcDisabler.Value, function(CallBack)
			...
			task.spawn(function()
				...
				repeat
					...
					if IsAlive(LocalPlayer) and Settings.Scaffold.Value == true then
						for i = 1, 2 do
							local ScaffoldCFrame = LocalPlayer.Character.HumanoidRootPart.Position + (LocalPlayer.Character.Humanoid.MoveDirection * (i * 3.3)) - Vector3.new(0, (LocalPlayer.Character.HumanoidRootPart.Size.Y / 2) + (LocalPlayer.Character.Humanoid.HipHeight + 1.5), 0)
...
task.spawn(function()
	local InfFly = CreateToggle(BlatantTab, "InfiteFly", false, function(CallBack)
		task.spawn(function()
			InfFly()
		end)	
	end)
...
task.spawn(function()
	local NoPlacementCps = CreateToggle(UtilityTab, "NoPlacementCps", Settings.NoPlacementCps.Value, function(CallBack)
