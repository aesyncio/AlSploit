...
local SaveFileName = "AlSploitPublicUpdate8"
...
local CollectionServiceOre = CollectionService:GetTagged("ore")
...
task.spawn(function()
	local function KillAuraHitMobs()
		...
		for i, v in pairs(CollectionServiceDiamondGuardian) do
		...
		repeat	
			...
			if GetMatchState() ~= 0 and IsAlive(LocalPlayer) and Settings.Nuker.Value == true then
				if NearestBed then
					...
					if Magnitude > Settings.Nuker.Range and Settings.Nuker.BreakOres.Value == true then
						for i, v in pairs(CollectionServiceOre) do
							ReplicatedStorage.rbxts_include.node_modules:FindFirstChild("@easy-games"):FindFirstChild("block-engine").node_modules:FindFirstChild("@rbxts").net.out._NetManaged.DamageBlock:InvokeServer({
								["blockRef"] = {
									["blockPosition"] = GetServerPosition(v.Position)
								},
								["hitPosition"] = GetServerPosition(v.Position),
								["hitNormal"] = GetServerPosition(v.Position)
							})	
						end
					end
		...
		task.spawn(function()
			repeat
				...
				if Settings.AutoBank.Value == true then
					if IsAlive(LocalPlayer) and not IsNetworkOwner(LocalPlayer.Character.HumanoidRootPart) then
						SecureEnderChestItems()
					end

					if IsAlive(LocalPlayer) and LocalPlayer.Character:GetAttribute("Health") > 30 and IsNetworkOwner(LocalPlayer.Character.HumanoidRootPart) then
