...
local SaveFileName = "AlSploitPublicUpdate10"
...
local NearestDiamondGuardian, NearestDiamondGuardianDistance = nil, nil
...
function ShootProjectile(Item, Projectile, Slowdown)
	local NearestPlayer = NearestPlayer
	...
	if NearestPlayer then
		local Args = {
			...
			[8] = {["drawDurationSeconds"] = (Slowdown and 0.6 or 0.96), ["shotId"] = HttpService:GenerateGUID(false)},
			[9] = (game.Workspace:GetServerTimeNow() - math.random(0.105, 0.115))
		}
...
task.spawn(function()
	local function KillAuraHitMobs()	
		if IsAlive(LocalPlayer) and Settings.KillAura.Value == true then
			local DiamondGuardian, DiamondGuardianDistance = NearestDiamondGuardian, NearestDiamondGuardianDistance
		...
		repeat	
			...
			if GetMatchState() ~= 0 and IsAlive(LocalPlayer) and Settings.Nuker.Value == true then
				if NearestBed then
					...
					if Magnitude > Settings.Nuker.Range and Settings.Nuker.BreakOres == true then
						local NearestOre = NearestOre
...
task.spawn(function()
	repeat		
		...
		if Settings.Aimbot.Value == true and IsAlive(LocalPlayer) and NearestPlayer and not NearestPlayer:FindFirstChildOfClass("ForceField") then	
			...
			if BestBow and IsAlive(LocalPlayer) then
				if Settings.Aimbot.ToolCheck == true then
					if GetItem(BestBow.itemType) and GetItem("arrow") then
						...
						if Bow then					
							ShootProjectile(ReplicatedStorage.Inventories:FindFirstChild(LocalPlayer.Name):FindFirstChild(BestBow.itemType), "arrow", (BestBow.itemType:lower():find("tactical") or BestBow.itemType:lower():find("cross") and true or false))
						end
					end
				end
			...
			v.Character.Humanoid.HealthChanged:Connect(function(Health)
				if Health <= 0 and v.Team ~= LocalPlayer.Team and v ~= LocalPlayer and Settings.AutoToxic.Value == true then
					...
					if Randomized == 0 then
						KillMessage = "You are so bad u died. L" .. v.Name .. " | AlSpoit on top"
					end

					if Randomized ~= 0 then
						KillMessage = "I could probably win without cheats. L" .. v.Name .. " | AlSpoit on top"
					end

					if Randomized == 2 then
						KillMessage = "Clowns are the only ones we kill. L" .. v.Name .. " | AlSpoit on top"
					end

					if Randomized == 3 then
						KillMessage = "Rats. Imagine dying in a block game. L" .. v.Name .. " | AlSpoit on top"
					end

					if Randomized == 4 then
						KillMessage = "I hack for fun. Too bad your fun is ruined. L" .. v.Name .. " | AlSpoit on top"
					end

					if Randomized == 5 then
						KillMessage = "I ruined a good day for a good reason. L" .. v.Name .. " | AlSpoit on top"
					end
	...
	local AutoBank = CreateToggle(UtilityWindow, "AutoBank", Settings.AutoBank.Value, function(CallBack)
		...
		task.spawn(function()
			repeat
				...
				if IsAlive(LocalPlayer) then
					if NearestNpc and IsNetworkOwner(LocalPlayer.Character.HumanoidRootPart) and LocalPlayer.Character:GetAttribute("Health") >= 30 and Settings.AutoBank.Value == true then
						CollectEnderChestItems()
					end

					if Settings.AutoBank.Value == true and not NearestNpc or not IsAlive(LocalPlayer) or not IsNetworkOwner(LocalPlayer.Character.HumanoidRootPart) or LocalPlayer.Character:GetAttribute("Health") < 30 then
...
task.spawn(function()
	repeat 
		...
		task.spawn(function()
			if IsAlive(LocalPlayer) then
				NearestDiamondGuardian, NearestDiamondGuardianDistance = FindNearestDiamondGuardian()
