...
function Fly()
			...
			repeat
				...
				if Flying == true and IsAlive(LocalPlayer) then
					...
					if (tick() - FlyGroundTime) >= 2 and Flying == true then
						local OldPosition = LocalPlayer.Character.HumanoidRootPart.Position.Y
						...
						if Raycast and IsAlive(LocalPlayer) then 
							...
							if OldPosition ~= -99999 and IsAlive(LocalPlayer) then 
								local Args = {LocalPlayer.Character.HumanoidRootPart.CFrame:GetComponents()}
								Args[2] = OldPosition
	...
	if Flying == false and LocalPlayer.CharootPart:FindFirstChild("FlyBodyVelocity"):Destroy()
		FlyGroundTime = tick()
	end

	if not IsAlive(LocalPlayer) aacter.HumanoidRootPart:FindFirstChild("FlyBodyVelocity") then
		LocalPlayer.Character.HumanoidRnd LocalPlayer.Character.HumanoidRootPart:FindFirstChild("FlyBodyVelocity") then
