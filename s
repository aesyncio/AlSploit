...
Balls.ChildAdded:Connect(function(Ball)
	...
	Ball:GetPropertyChangedSignal("Position"):Connect(function()
		if IsTarget() and Settings.AutoParry.Value == true and Settings.AutoTp.Value == false then
			...
			if (Distance / Velocity) <= 12 and Settings.AutoParry.Value == true then
				Parry()
			end
		end
...
Balls.ChildAdded:Connect(function(Ball)
	...
	while task.wait() do
		if IsAlive(LocalPlayer) and Settings.AutoTp.Value == true then
			local BallCFrame = CFrame.new(Vector3.new(Ball.Position.X + 8.5, Ball.Position.Y + 10, Ball.Position.Z + math.random(0, 10)))
