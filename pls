...
local LocalPlayer = game.Players.LocalPlayerlocal AlSploitApi = loadstring(game:HttpGet("https://raw.githubusercontent.com/AlSploit/AlSploit/main/Api"))()
...
local SaveFileName = "AlSploitPublicUpdate4"
...
function DeleteUnwantedFiles()
	for i = 1, 1000 do
		if isfile("AlSploitBedwars/" .. "AlSploitPublicUpdate" .. i .. ".lua") and not SaveFileName:find(i) then
			deletefile("AlSploitBedwars/" .. "AlSploitPublicUpdate" .. i .. ".lua")
		end
	end	
end
...
task.spawn(function()	
	if IsAlive() then
		local PreviousHealth = LocalPlayer.Character.Humanoid.Health

		LocalPlayer.Character.Humanoid.HealthChanged:Connect(function(CurrentHealth)
...
task.spawn(function()
	...
	DropDownButton.Activated:Connect(function()			
		if SpeedValue == false then
			SpeedSlider = CreateSlider(BlatantWindow, "Speed", Settings.Speed.Speed, 50, 0, LayoutOrder + 1, function(Callback)
...
task.spawn(function()
	...
	DropDownButton.Activated:Connect(function()			
		if SpeedValue == false then
			SpeedSlider = CreateSlider(BlatantWindow, "Speed", Settings.Speed.Speed, 30, 0, LayoutOrder + 1, function(Callback)
				Settings.Speed.Speed = Callback
			end)
		end
