...
task.spawn(function()
	...
	local KillAuraKeyBindValue = false
	local KillAuraKeyBind
	
	task.spawn(function()
		DropDownButton.Activated:Connect(function()
			KillAuraKeyBindValue = not KillAuraKeyBindValue
		
			if KillAuraKeyBindValue == true then
				KillAuraKeyBind = CreateKeyBind(CombatTab, Settings.KillAura.KeyBind, LayoutOrder + 1, function(CallBack)
			...
			if KillAuraKeyBindValue == false then
				KillAuraKeyBind:Destroy()
			end
		end)	
	end)
end)
