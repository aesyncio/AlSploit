...
function TweenToNearestPlayer()
	if IsAlive(LocalPlayer) then
		...
		if NearestPlayer then
			...
			local PlayerTpTween = TweenService:Create(LocalPlayer.Character.PrimaryPart, TweenInformation, {CFrame = NearestPlayer.Character.HumanoidRootPart.CFrame + Vector3.new(0, 2, 0)})
...
task.spawn(function()
	repeat
		...
		if Settings.ChatSpammer.Value == true then
			...
			if Randomized == 5 then
				KillMessage = "Kids on bedwars just keep getting worst. | AlSploit On Top"
			end
