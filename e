...
task.spawn(function()
	repeat
		...
		if Settings.AutoClicker.Value == true and IsAlive(LocalPlayer) and GetMatchState() ~= 0 then
			task.spawn(function()
				if BlockPlacementController.blockPlacer.clientManager then
		...
		if Settings.AntiVoid.Value == true then
			task.spawn(function()
				...
				AntiVoidPart.Touched:Connect(function()
					if IsAlive(LocalPlayer) and AntiVoidPart and LocalPlayer.Character.PrimaryPart.Position.Y <= game.Workspace.AntiVoidPart.Position.Y then
						for i = 1, 3 do
							if IsAlive(LocalPlayer) then
								LocalPlayer.Character.PrimaryPart.Velocity = Vector3.new(LocalPlayer.Character.PrimaryPart.Velocity.X, 100, LocalPlayer.Character.HumanoidRootPart.Velocity.Z)
							end
						end
					end
				end)
