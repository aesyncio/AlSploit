...
local Executed = loadstring(game:HttpGet("https://raw.githubusercontent.com/AlSploit/AlSploit/main/Executed"))()
...
local SaveFileName = "AlSploitPublicUpdate9"
...
local BedwarsSwords = AlSploitApi.BedwarsSwords
...
function FindNearestDiamondGuardian(MaxDistance)
	for i, v in next, CollectionServiceDiamondGuardian do
		local NearestDiamondGuardianDistance = MaxDistance or math.huge
...
task.spawn(function()
	local function KillAuraHitMobs()	
		local DiamondGuardian = FindNearestDiamondGuardian(Settings.KillAura.Range)

		if Settings.KillAura.Value == true and DiamondGuardian then
			if GetMatchState() ~= 0 and IsAlive(LocalPlayer) and DiamondGuardian and DiamondGuardian.PrimaryPart then
		...
		if Settings.Aimbot.Value == true and IsAlive(LocalPlayer) and NearestPlayer and not NearestPlayer:FindFirstChildOfClass("ForceField") then	
			...
			if BestBow and IsAlive(LocalPlayer) then
				if Settings.Aimbot.ToolCheck == true then
					if GetItem(BestBow.itemType) then
						local Bow = GetItem(BestBow.itemType)

						if Bow then					
							ShootProjectile(BestBow.itemType, "arrow")
						end
					end
				end
...
task.spawn(function()
	repeat
		task.wait(100 / Settings.ChatSpammer.Speed * 2)
