local WhitelistedUsers = loadstring(game:HttpGet("https://raw.githubusercontent.com/AlSploit/AlSploit/main/Whitelisted"))()
	...
	for i, v in next, PlayerService:GetPlayers() do
		if IsAlive(v) then
			v.Character.Humanoid.HealthChanged:Connect(function(Health)
				...
				if Settings.GodMode.Value == true and Health > 30 and GodModed == true then
					InfiniteFlyValue = false
					GodModed = false
				end
				
				if Settings.GodMode.Value == true and Health <= 30 and GodModed == false and InfiniteFlyValue == false then
